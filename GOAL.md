...
A conversation is marked **error** if the agent itself failed at some point. A tool that failed but the agent recovered from does **not** mark the whole conversation as failed — that's normal agent behavior, not a conversation error.

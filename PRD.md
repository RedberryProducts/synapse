...
- **Installer registration is idempotent and migrates old installs.** Re-running the command preserves host application edits, never duplicates the guarded block, and removes the old unconditional `bootstrap/providers.php` entry. Only the complete generated block in executable PHP counts as an existing installation; an ambiguous registration causes an actionable error before the application provider or bootstrap registration is changed.
